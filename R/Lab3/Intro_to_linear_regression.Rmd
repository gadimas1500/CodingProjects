...
1.  What type of plot would you use to display the relationship between `runs` 
    ...
    variable `at_bats` as the predictor. (2 pts) Does the relationship look linear? (1 pt) If 
    you knew a team's `at_bats`, would you be comfortable using a linear model 
    to predict the number of runs? (1 pt) [4 pts]
...
2.  Looking at your plot from the previous exercise, describe the relationship 
    between these two variables. Make sure to discuss the form (1 pt), direction (1 pt), and 
    strength (1 pt) of the relationship as well as any unusual observations. [3 pts]
...
3.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    squares that you got? (1 pt) How does it compare to other attempts in your group? (1 pt) [2 pts]
...
4.  Fit a new model that uses `homeruns` to predict `runs`. Using the estimates 
    from the R output, write the equation of the regression line. (2 pts) What does the 
    slope tell us in the context of the relationship between success of a team 
    and its home runs? (2 pts) [4 pts]
...
5.  If a team manager saw the least squares regression line and not the actual 
    data, how many runs would he or she predict for a team with 5,579 at-bats? (1 pt)
    Is this an overestimate or an underestimate (1 pt), and by how much (1 pt)? In other 
    words, what is the residual for this prediction? (1 pt) [4 pts]
...
6.  Is there any apparent pattern in the residuals plot? (1 pt) What does this indicate
    about the linearity of the relationship between runs and at-bats? (1 pt) [2 pts]
...
7.  Based on the histogram and the normal probability plot, does the nearly 
    normal residuals condition appear to be met? [2 pts]
...
-   Choose another traditional variable from `mlb11` that you think might be a 
    ...
    a linear model (2 pts). At a glance, does there seem to be a linear relationship? (2 pts) [4 pts]
...
-   Now that you can summarize the linear relationship between two variables, 
    ...
    traditional variables. Which variable best predicts `runs`? (3 pts) Support your 
...
-   Now examine the three newer variables. These are the statistics used by the 
    ...
    or less effective at predicting runs that the old variables? (1 pt) Explain using 
    appropriate graphical and numerical evidence. (1 pt) Of all ten variables we've 
    analyzed, which seems to be the best predictor of `runs`? (2 pt) Using the limited 
    (or not so limited) information you know about these baseball statistics, 
    does your result make sense? (2 pts) [6 pts]

-   Check the model diagnostics for the regression model with the variable you 
    decided was the best predictor for runs. [6 pts]
